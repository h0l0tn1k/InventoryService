...
public class DeviceController2 implements DevicesApi {

	private DeviceFacade deviceFacade;
	final static Logger logger = LoggerFactory.getLogger(DeviceController2.class);

	@Autowired
	public DeviceController2(DeviceFacade deviceFacade) {
	...
	public ResponseEntity<Device> getDevice(@ApiParam(required = true) @PathVariable("deviceId") Long deviceId) {
		...
		return ResponseEntity.ok(deviceFacade.getDevice(deviceId));
	}
